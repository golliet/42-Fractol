...
/*   Updated: 2018/08/07 11:16:05 by golliet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (var->x < WIN_X)
	{
		...
		while (var->y < WIN_Y)
		{
					...
					* var->z_imag) < 4 && var->i < img->iteration)
			{
				...
				var->z_imag = 2 * fabs(var->z_imag) * fabs(var->tmp) + var->p_i;
				var->i++;
			}
			if (var->i == img->iteration)
				ft_change_color(img, var->x, var->y, (t_color){0, 0, 0});
			else
				ft_change_color(img, var->x, var->y, modulate_color(var->i,
												choose_color(set, var->i)));
			var->y++;
		}
		var->x++;
	}
